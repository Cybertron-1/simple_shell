...
ssize_t input_buf(information_tree *information, char **buffer, size_t *length_)
{
	...
	if (!*length_) /* if nothing left in the buffer, fill it */
	{
		...
		*buffer = NULL;
		...
		if (recall > 0)
		{
			if ((*buffer)[recall - 1] == '\n')
			{
				(*buffer)[recall - 1] = '\0'; /* remove trailing newline */
				recall--;
			}
			...
			/* if (_string_tcharacter(*buffer, ';')) is this a command chain? */
			{
				*length_ = recall;
...
ssize_t capture_input(information_tree *information)
{
	...
	if (recall == -1) /* EOF */
		return (-1);
	if (length_) /* we have commands left in the chain buffer */
	{
		iteratej = iteratei; /* init new iterator to current buffer position */
		finalpointer = buffer + iteratei; /* get pointer for return */

		verify_chase(information, buffer, &iteratej, iteratei, length_);
		while (iteratej < length_) /* iterate to semicolon or end */
		{
		...
		iteratei = iteratej + 1; /* increment past nulled ';'' */
		if (iteratei >= length_) /* reached end of buffer? */
		{
			iteratei = length_ = 0; /* reset position and length */
			information->cmd_buf_type = COMMANDLINE_NORMILIZATION;
		}

		*buf_p = finalpointer; /* pass back pointer to current command position */
		return (_string_length(finalpointer)); /* return length of current command */
	}

	*buf_p = buffer; /* else not a chain, pass back buffer from _getline() */
	return (recall); /* return length of buffer from _getline() */
}
